...
## 21. Passo a passo para enviar ao GitHub
...
```bash
git commit -m "Criando sistema de manutencao de contas"
```
...
2. Escolha um nome, por exemplo:

```text
sistema-manutencao-contas
```

3. Deixe como público ou privado, conforme a orientação do professor.
...
Depois de criar o repositório, o GitHub mostrará comandos parecidos com estes:

```bash
git branch -M main
git remote add origin https://github.com/seu-usuario/sistema-manutencao-contas.git
...
### Passo 7: Enviar o link da atividade

Depois que os arquivos estiverem no GitHub, copie o link do repositório e envie na entrega da atividade.

Exemplo:

```text
https://github.com/seu-usuario/sistema-manutencao-contas
```
...
Este projeto atende ao enunciado porque utiliza arquivo binário com registros de tamanho fixo e implementa todas as operações solicitadas.

O programa permite acessar posições específicas com `fseek()`, ler registros com `fread()`, gravar registros com `fwrite()` e restaurar a leitura do arquivo com `rewind()`.

Além disso, o sistema possui um menu completo, permitindo ao usuário cadastrar, consultar, atualizar, encerrar, listar e repetir a leitura dos clientes armazenados no arquivo binário.
